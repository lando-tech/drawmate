...
def create_node_meta_left(left_nodes: dict) -> list[NodeMetaData]:
    ...
    for key, node in left_nodes.items():
        ...
        node_meta = drawmate.create_node_metadata(
            node_attributes_left, int(col), int(row), "left"
        )
...
def create_node_meta_right(right_nodes: dict) -> list[NodeMetaData]:
    ...
    for key, node in right_nodes.items():
        ...
        node_meta = drawmate.create_node_metadata(
            node_attributes_right, int(col), int(row), "right"
        )
        # print(node_meta.__COLUMN_INDEX__, node_meta.__ROW_INDEX__)
...
def calculate_node_x_offset(
    previous_node_x: int, matrix_width: int, node_col: int, side: str
):

    if node_col == 0 and side == "right":
        return (previous_node_x + matrix_width) + NodeAttributes.x_spacing
    if node_col == 0 and side == "left":
        return previous_node_x - NodeAttributes.x_spacing

    if side == "right":
        return (previous_node_x + matrix_width) + NodeAttributes.x_spacing * node_col
    else:
        return previous_node_x - (
            (NodeAttributes.x_spacing * node_col) - NodeAttributes.width
        )
...
def create_nodes(attributes: dict, node_meta_array: list[NodeMetaData]):
    ...
    for node_meta in node_meta_array:
        if node_meta.__SPANNING_NODE__:
            continue
        attributes["x"] = calculate_node_x_offset(
            matrix_dims.x,
            matrix_dims.width,
            node_meta.__COLUMN_INDEX__,
            node_meta.__SIDE__,
        )
        attributes["y"] = calculate_node_y_offset(
            matrix_dims.y, node_meta.__ROW_INDEX__
        )

        print(
            f"Node {node_meta.__ID__} at (x = {attributes['x']}, y = {attributes['y']})"
        )
